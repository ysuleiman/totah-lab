...
ALTER TABLE docking.docking_pose
    ...
    ADD COLUMN IF NOT EXISTS ligand_label varchar(128);
...
CREATE INDEX IF NOT EXISTS docking_pose_chemflow_selectivity_v2_idx
    ON docking.docking_pose (
        receptor_id, ligand_id, vina_score, id
    )
    INCLUDE (ligand_label)
    WHERE source_system = 'chemflow3';
...
COMMENT ON COLUMN docking.docking_pose.ligand_label IS
    'Human-readable ligand identifier supplied by the source system.';
