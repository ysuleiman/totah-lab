...
-- Adds alpha_sphere_count and geometry_basis to docking.pocket_summary_mv.
-- The view is dropped and recreated because materialized views cannot
-- gain columns in place; the definition is the pre-existing one (see
-- pg_get_viewdef('docking.pocket_summary_mv')) plus a sphere_stats CTE.
-- pocket_atom_residue_idx belongs to docking.pocket_atom, not to this
-- view, so only the four pocket_summary_mv_* indexes are recreated.
