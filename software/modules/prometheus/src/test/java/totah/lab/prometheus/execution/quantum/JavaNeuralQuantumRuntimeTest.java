...
final class JavaNeuralQuantumRuntimeTest {
    ...
    @Test void generalVectorForceIsExactBitRegisteredAndReusedWithoutRecomputation()throws Exception{AtomicInteger calls=new AtomicInteger();QuantumBackend delegate=new JavaNeuralQuantumBackend();QuantumBackend counted=new QuantumBackend(){public String backendId(){return delegate.backendId();}public QuantumBackendCapabilities capabilities(){return delegate.capabilities();}public boolean supports(QuantumExecutionRequest r){return delegate.supports(r);}public QuantumResult execute(QuantumExecutionRequest r)throws EvidenceExecutionException{calls.incrementAndGet();return delegate.execute(r);}};QuantumExecutionRequest request=generalForceRequest();var runtime=new JavaNeuralQuantumRuntime(new QuantumExecutionService(new QuantumBackendSelector(List.of(counted))),new GeneratedEvidenceRegistry(temp.resolve("vector-force-registry")));var first=runtime.executeOrReuse(request);assertEquals(JavaNeuralQuantumRuntime.Disposition.GENERATED_NEW,first.disposition());assertTrue(first.generatedResult().orElseThrow().forceIsNegativeGradient(0));assertEquals("GENERAL_ANALYTIC_DIFFERENTIAL_SWCT",first.generatedResult().orElseThrow().executionProvenance().get("force_estimator"));assertEquals(JavaNeuralQuantumRuntime.Disposition.REUSE_EXISTING,runtime.executeOrReuse(request).disposition());assertEquals(1,calls.get());String artifact=first.generatedResult().orElseThrow().artifactChecksums().keySet().iterator().next();String json=java.nio.file.Files.readString(temp.resolve("general-force-work").resolve(artifact));assertTrue(json.contains("raw_ieee754_bits"));assertTrue(json.contains("hartree_per_bohr"));}
    ...
    private QuantumExecutionRequest generalForceRequest(){double r=1.4;var geometry=new CartesianGeometry(List.of(new CartesianGeometry.Atom("H",0,0,-r/2),new CartesianGeometry.Atom("H",0,0,r/2)),"bohr");var molecule=new MoleculeIdentity("prometheus-general-force-h2-fixture","General vector-force H2 fixture","H2");var specification=new CalculationSpecification("step2-force-test","Step-2 vector force fixture",molecule,new GeometryIdentity("c".repeat(64),2),0,1,new QmProtocol("PROMETHEUS_NEURAL_VMC","NEURAL_STATE_V1","none","isolated",false,"Prometheus",JavaNeuralRuntimePolicy.BACKEND_VERSION),List.of(),CalculationType.FORCE_EVALUATION,List.of("ABSOLUTE_ENERGY","CARTESIAN_GRADIENT","CARTESIAN_FORCE","wavefunction=general-slater-jastrow-atom-centered-v1","force=general-analytic-differential-swct-v1"),List.of("finite","force=-gradient","canonical atom order"),DatasetRole.DEVELOPMENT,CostEstimate.zero());return new QuantumExecutionRequest(specification,geometry,"d".repeat(64),QuantumSolverMode.VARIATIONAL_QUANTUM_STATE,QuantumExecutionRequest.energyAndForces(),new QuantumExecutionOptions(1,1024,temp.resolve("general-force-work"),List.of(),Optional.empty()));}
}
