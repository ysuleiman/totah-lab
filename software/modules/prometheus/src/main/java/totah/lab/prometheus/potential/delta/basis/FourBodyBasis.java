...
public final class FourBodyBasis implements ManyBodyBasis {
    public enum Kind { TORSION_FOURIER, ANGLE_PAIR, ANGLE_PAIR_SHARED_FOURTH }
    ...
    @Override public BasisEvaluation evaluate(QuantumCoordinates coordinates){int variables=coordinates.atomCount()*3;double[] values=new double[dimension()];double[][][] gradients=new double[dimension()][coordinates.atomCount()][3];int offset=0;
        for(Motif motif:motifs){D[][] p=new D[4][3];int[] atoms={motif.first(),motif.second(),motif.third(),motif.fourth()};for(int a=0;a<4;a++)for(int q=0;q<3;q++)p[a][q]=D.variable(coordinates.coordinate(atoms[a],q),atoms[a]*3+q,variables);
            List<D> generated=switch(motif.kind()){case TORSION_FOURIER->torsion(p);case ANGLE_PAIR->anglePair(p);case ANGLE_PAIR_SHARED_FOURTH->anglePairSharedFourth(p);};for(D feature:generated){values[offset]=feature.value;for(int atom=0;atom<coordinates.atomCount();atom++)for(int q=0;q<3;q++)gradients[offset][atom][q]=feature.gradient[atom*3+q];offset++;}}
    ...
    private static List<D> anglePairSharedFourth(D[][]p){D left=angleCos(p[0],p[1],p[3]),right=angleCos(p[2],p[1],p[3]);List<D>out=new ArrayList<>();for(int l=1;l<=2;l++)for(int m=1;m<=2;m++)out.add(mul(legendre(left,l),legendre(right,m)));return out;}
