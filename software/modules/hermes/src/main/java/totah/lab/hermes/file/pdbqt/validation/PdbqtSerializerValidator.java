...
public final class PdbqtSerializerValidator {
    public PdbqtValidationReport validate(PdbqtFlexibleReceptor input) {
        ...
        for(var residue:input.flexibleResidues()){
            Map<String,PdbqtFragment> fragments=new HashMap<>();
    ...
    private void validateAtom(totah.lab.hermes.file.pdbqt.PdbqtAtomReference atom,Set<Integer> indices,List<PdbqtValidationIssue> issues,String location){
        ...
        if(!PdbqtTypes.isSupported(atom.ad4Type()))issues.add(error(PdbqtValidationCode.MISSING_AD4_TYPE,"AD4 type is missing or unsupported.",location));
    }
