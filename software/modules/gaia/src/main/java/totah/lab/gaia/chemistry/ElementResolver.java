...
public final class ElementResolver {
    ...
    public static String resolve(Atom atom, Residue residue) {
        boolean monoatomicResidue = residue != null && residue.getAtomCount() == 1;
        return resolve(atom, monoatomicResidue);
    }

    public static String resolve(Atom atom, boolean monoatomicResidue) {
        if (atom == null) {
            return "C";
        }
        Element element = atom.getElement();
        if (element != null && !element.isUnknown()) {
            return canonicalize(element.symbol());
        }
