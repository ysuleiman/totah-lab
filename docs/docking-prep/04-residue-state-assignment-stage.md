...
- Assigns bundled O-sulfo tyrosine templates from PREPI resources: `NTYS` for
  N-terminal `TYS`, `TYS` for internal `TYS`, and `CTYS` for C-terminal `TYS`.
...
`TYS` is not inferred or generated by this stage. It is supported only when the
loaded structure already contains correctly named O-sulfo tyrosine atoms. The
bundled TYS templates come from `biology-12-00824-s001`, whose README describes
ff14SB-compatible RESP charges generated via RED at HF/6-31G* and gaff2-derived
missing bonded parameters, with the authors' caveat that the set is not
extensively tested.
...
- `TYS` terminal and internal template assignment.
