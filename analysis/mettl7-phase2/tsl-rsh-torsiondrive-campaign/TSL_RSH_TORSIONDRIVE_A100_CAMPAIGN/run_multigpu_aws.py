#!/usr/bin/env python3
"""AWS 8xA100 execution wrapper; scientific decisions remain in the sealed runner."""
...
DEFAULT_EBS_ROOT = Path("/mnt/tsl-rsh-ebs/torsiondrive")
MAX_GPU_WORKERS = 8
EXECUTION_AXIS = TorsionAxis.PHI
...
def verify_ebs(root: Path):
    root.mkdir(parents=True, exist_ok=True)
    result = subprocess.run(["findmnt", "-n", "-o", "TARGET,SOURCE,FSTYPE", "-T", str(root)],
                            text=True, capture_output=True, check=True)
    target = result.stdout.split()[0]
    if target == "/":
        raise RuntimeError("results root resolves to root/ephemeral filesystem; persistent EBS required")
    probe = root / ".persistence_probe"
    probe.write_text("EBS_PERSISTENCE_OK\n")
...
def run_phi(results_root: Path, workers: int, s3_uri: str | None):
    controller = load_serial(); manifest = controller.verify_package()
    if (manifest["execution_torsions"][0] != EXECUTION_AXIS.value
            or manifest["chi_execution_authorized"]):
        raise RuntimeError(f"{EXECUTION_AXIS.value}-only first execution contract violated")
    ebs = verify_ebs(results_root); controller.RESULTS = results_root
    ...
    if state_path.is_file():
        ...
        state = {"schema": "torsiondrive-wavefront-state-v1",
                 ...
                 "aws_execution": {"max_gpu_workers": workers, "ebs_mount": ebs}}
...
def main():
    ...
    parser.add_argument("--run-phi", action="store_true"); parser.add_argument("--status", action="store_true")
    parser.add_argument("--verify-shutdown", action="store_true"); parser.add_argument("--workers", type=int, default=8)
    parser.add_argument("--results-root", type=Path, default=DEFAULT_EBS_ROOT); parser.add_argument("--s3-uri")
    ...
    if args.run_phi: print(json.dumps(run_phi(args.results_root, args.workers, args.s3_uri), indent=2)); return
    parser.error("choose --run-phi, --status, --verify-shutdown, or --worker")
