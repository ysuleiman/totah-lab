...
scheduler = load("mg_scheduler", HERE / "run_multigpu_psi.py")
...
assert "run_psi" in scheduler.run_psi.__name__ and not hasattr(scheduler, "run_chi") and not hasattr(scheduler, "run_phi")
...
source = (HERE / "run_multigpu_psi.py").read_text()
...
assert source.index('outputs = [future.result() for future in futures]') < source.index(
    'cells, queue = deterministic_reduce', source.index('def run_psi'))
