...
| 12 | Set-like scientific identity fields lacked canonical ordering; **CONFIRMED=true** | Reordering equivalent `constraints`, `requiredOutputs`, or `acceptanceGates` changed the calculation checksum/scientific identity even though membership was unchanged. `requiredOutputs` was already present; omission was not the defect. | Encounter order leaked into the identity of fields whose documented semantics are set-like. | The three fields are deduplicated and lexicographically canonicalized in `CalculationSpecification` and consumed canonically by `QuantumScientificIdentity`. Tests prove reorder invariance and removal/change sensitivity. | Equivalent requests could acquire different identities and miss valid reuse; requests with different members remain distinct. No numerical QM result changed. **RECOMPUTATION_REQUIRED=false; identity registries may require canonical-key reconciliation**. |
...
The persistence boundary itself is fail-closed and checksum-verified, and
`DeltaModelTrainer` exposes that boundary. There is currently **no production
fitter implementation wired through `DeltaModelTrainer`**: it provides the
basis-preflight gate and `persistSuccessfulFit(...)`, but no `train(...)` or
equivalent fitting operation. Consequently production-wide enforcement remains
false rather than being overstated. The first actual fitter implementation must
make a checksum-verified `FitArtifactWriter.Receipt` part of its success
contract; it must not expose or report successful fitted state before that
receipt exists.
