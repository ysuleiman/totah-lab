...
def main() -> None:
    ...
    request = {"modelFamily": "FROZEN_AMBER_ADDITIVE_PLUS_LOW_ORDER_CROSS_TERMS", "modelVersion": "TSL_RSH_GPU60_CROSS_COUPLING_V1", "basisDefinition": json.dumps({"frozen_parent": "6d139cfb94130a660b8916fca280caa846883af2 additive fit", "centers": centers, "selected_terms": selected_terms}, sort_keys=True), "parameterNames": parameter_names, "parameterUnits": parameter_units, "designMatrix": final_design.tolist(), "targets": target.tolist(), "rowWeights": weights.tolist(), "frozenParameters": {"additive_artifact_sha256": json.loads((REPRESENTABILITY / "FIT_RECEIPT.json").read_text())["artifact_sha256"]}, "objectiveDefinition": "Same frozen 0.5 normalized energy + 0.5 normalized all-Cartesian-force objective, now applied to residuals after the immutable additive model", "objectiveWeights": {"energy_block": 0.5, "force_block": 0.5, "energy_row_weight": energy_weight, "force_row_weight": force_weight}, "trainingIds": train_ids, "validationIds": list(frozen["VALIDATION_IDS"]), "normalizationState": {"energy_scale_kcal_mol": repr(energy_scale), "force_scale_kcal_mol_angstrom": repr(force_scale), "training_only": "true"}, "optimizerConfiguration": {"algorithm": "twice-reorthogonalized modified Gram-Schmidt QR", "regularization": "NONE", "rank": str(rank)}, "seed": 0, "sourceDatasetChecksums": source_checksums, "codeCommitSha": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip()}
