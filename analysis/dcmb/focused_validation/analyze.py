...
def main():
 ...
 pose_rows=[]; tsl_rows=[]
 for key,entries in entries_by.items():
  para,cid=key; rec=pdb_atoms(RECEPTOR_PDB[para]); protein=[a for a in rec if a['res']!='SAM']; sam=xyz([a for a in rec if a['res']=='SAM']); sphere=clouds[para]
 ...
 parent=lambda c:'BA' if c=='BA' else ('DCMB' if c.startswith('DCMB') else ('CONH' if c.startswith('CONH') else '24DCMB'))
 parent_rows=[]
 for para in ('7A','7B'):
  for par in ('BA','DCMB','24DCMB','CONH'):
 ...
 (HERE/'analysis_manifest.json').write_text(json.dumps({'clustering':'connected components of direct same-atom-order pose RMSD <2.0 A; family count not prespecified','superpocket':'WT7B accepted 197-sphere pocket; rigid CA-homology transfer to WT7A','superpocket_transfer':transform,'on_site':'at least 70% ligand heavy atoms within 4 A of homologous superpocket spheres','substrate_facing':'at least one ligand atom inside the 1.7 A envelope of any retained productive TSL state','escape':'on-site and zero atoms inside union productive-TSL 1.7 A envelope','shared_volume':'0.5 A grid, same radius definitions as prior DCMB/TSL analysis','protein_contacts':'residue fingerprint with any protein heavy atom within 4.5 A','tsl_states':'reused unchanged: five 7A relaxed productive states and six 7B fixed productive states','evidence_labels':{'DCMB':'direct modern recombinant METTL7A inhibition and reported METTL7B non-inhibition','CONH':'historical TMT/PNMT inhibitor comparator; no direct METTL7A-vs-METTL7B activity located','BA_and_24_isomer':'historical benzylamine/PNMT SAR controls; no direct METTL7 activity retained'}},indent=2)+'\n')
